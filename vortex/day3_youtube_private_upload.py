...
PACKAGE_NAME = "youtube_video_3_of_3_package.json"
...
def _upload_marker(preview_gw: int) -> str:
    return f"fpl-vortex-auto-gw{preview_gw:02d}-video-3-of-3"
...
def _validate_package(output_root: Path) -> dict[str, Any]:
    ...
    if metadata.get("series") != {"part": 3, "total": 3}:
        raise RuntimeError("YouTube package must be VIDEO 3/3")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Upload the validated FPL VORTEX VIDEO 3/3 package to the verified "
